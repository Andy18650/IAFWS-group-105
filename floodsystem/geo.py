...
def rivers_by_station_number(stations, N):
    ...
    return [magicTuple[1] for magicTuple in zip(range(1,len(stations_by_river(stations))+1),sorted_by_key([(river,len(stationList)) for (river,stationList) in list(stations_by_river(stations).items())],1)[::-1],[sorted_by_key([(river,len(stationList)) for (river,stationList) in list(stations_by_river(stations).items())],1)[-N][1]]*len(stations_by_river(stations))) if magicTuple[0] <= N or magicTuple[1][1] == magicTuple[2]]
